...
Editconfig/config.yaml` to customize:
...
The dashboard will open athttp://localhost:8501` and display:
...
- Processed Data:experiments/data/processed/` ordata/processed/`
...
- Ensureexperiments/__init__.py` exists

File not found errors
- Run training first to generate visualizations
- Check that data files exist indata/processed/` orexperiments/data/processed/`

CUDA out of memory
- Reducebatch_size` in config
- Use CPU:--device cpu`

Dashboard shows "No metrics found"
- Train at least one model first
- Check thatresults/_metrics.csv` files exist
...
Built with for Energy Forecasting
